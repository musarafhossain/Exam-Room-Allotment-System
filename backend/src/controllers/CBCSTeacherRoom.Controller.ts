...
import { CBCSTeacherRoomModel } from "../models";
import { Types } from "mongoose";

class CBCSTeacherRoomController {
    getTeacherRooms = async (req: Request, res: Response) => {
        const rooms = await CBCSTeacherRoomModel.find();

        const total = await CBCSTeacherRoomModel.countDocuments();

        res.json({
            success: true,
            message: "CBCS Teacher rooms fetched successfully",
            items: rooms,
            total
        });
    }
    ...
    getTeacherRoom = async (req: Request, res: Response) => {
        ...
        const room = await CBCSTeacherRoomModel.findById(id);
    ...
    createTeacherRoom = async (req: Request, res: Response) => {
        ...
        await CBCSTeacherRoomModel.deleteMany({});
        
        const created = await CBCSTeacherRoomModel.insertMany(newRecords);
    ...
    findTeacherRoom = async (req: Request, res: Response) => {
        try {
            ...
            let rooms = await CBCSTeacherRoomModel.find(query);
    ...
    bulkDeleteTeacherRoom = async (req: Request, res: Response) => {
        try {
            ...
            if (ids) {
                await CBCSTeacherRoomModel.deleteMany({ _id: { $in: ids } });
            }
...
export default new CBCSTeacherRoomController();
